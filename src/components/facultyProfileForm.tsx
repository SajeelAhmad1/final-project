...
const FacultyProfileForm: React.FC = () => {
  ...
  useEffect(() => {
    if (!session?.user?.faculty) return;
    setProfile(session.user.faculty);
    if (session.user.faculty.imageUrl) {
      setImagePreview(session.user.faculty.imageUrl);
    }
  }, [session]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/profile/faculty-profile", {
        method,
